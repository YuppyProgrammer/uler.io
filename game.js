...
let W = 1200;
let H = 700;
const LANDSCAPE_SIZE = [1200, 700];
const PORTRAIT_SIZE = [700, 1200];
const CELL = 20;
...
let JOYSTICK_PUSAT = [110, H - 110];
const JOYSTICK_RADIUS_LUAR = 88;
const JOYSTICK_RADIUS_KNOB = 38;
const JOYSTICK_DEADZONE = 18;

let BOOST_PUSAT = [W - 100, H - 100];
...
function resizeGame() {
  ...
  const size = portrait ? PORTRAIT_SIZE : LANDSCAPE_SIZE;
  ...
  JOYSTICK_PUSAT = [110, H - 110];
  BOOST_PUSAT = [W - 100, H - 100];

  gameWrap.classList.toggle("mobile-portrait", portrait);

  if (snake.length && (oldW !== W || oldH !== H)) {
    const scaleX = W / oldW;
    const scaleY = H / oldH;

    snake = snake.map(([x, y]) => [
      Math.max(0, Math.min(W - CELL, Math.round((x * scaleX) / CELL) * CELL)),
      Math.max(0, Math.min(H - CELL, Math.round((y * scaleY) / CELL) * CELL))
    ]);

    foods = foods.map(food => ({
      ...food,
      pos: [
        Math.max(0, Math.min(W - CELL, Math.round((food.pos[0] * scaleX) / CELL) * CELL)),
        Math.max(0, Math.min(H - CELL, Math.round((food.pos[1] * scaleY) / CELL) * CELL))
      ]
    }));
...
function makeFood(forbidden) {
  ...
  return {
    ...
    jenis: type,
    warna: type === "biasa"
      ? [...WARNA_BUAH_BIASA[randInt(WARNA_BUAH_BIASA.length)]]
      : null,
    bentuk: type === "biasa"
...
function drawFood(food, now) {
  ...
  if (type === "emas") {
    ...
    const fruitColor = food.warna || MERAH;
    const dark = fruitColor.map(c => Math.max(0, c - 45));
    const light = fruitColor.map(c => Math.min(255, c + 55));
    ...
    drawBasicShape(food.bentuk, cx, cy, size, rgb(dark), null, 2);
...
function addEatingEffect(x, y, color, now) {
  eatingEffects.push({ x, y, color: [...color], start: now });
}

function drawEatingEffects(now) {
  eatingEffects = eatingEffects.filter(effect => {
    const progress = (now - effect.start) / 280;
    if (progress >= 1) return false;

    const radius = 6 + progress * 18;
    ctx.strokeStyle = rgb(effect.color, 0.75 * (1 - progress));
...
function updateGame(now) {
  ...
  if (eaten) {
    ...
    addEatingEffect(pos[0] + CELL / 2, pos[1] + CELL / 2, color, now);
